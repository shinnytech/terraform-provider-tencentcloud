...
func resourceTencentCloudInstanceCreate(d *schema.ResourceData, meta interface{}) error {
	...
	for _, instanceType := range instanceTypeCandidates {
		...
		err = resource.Retry(5*readRetryTimeout, func() *resource.RetryError {
			...
			} else if instance != nil && *instance.InstanceState == CVM_STATUS_LAUNCH_FAILED {
				// 库存不足，继续尝试下一个实例类型，根据客服回答，此时无需担心实例状态
				return resource.NonRetryableError(cvmResourceInsufficientError)
			} else if instance != nil && *instance.InstanceState == CVM_STATUS_PENDING {
				// 创建中
				return resource.RetryableError(fmt.Errorf("cvm instance status is %s, retry...", *instance.InstanceState))
			} else if instance != nil && *instance.InstanceState != CVM_STATUS_RUNNING {
				// 其他状态，直接报错
	...
	// 如果所有实例类型都资源不足，直接报错
	if errors.Is(err, cvmResourceInsufficientError) {
		return fmt.Errorf("cvm 资源不足导致开机失败，尝试机型列表：%s", strings.Join(instanceTypeCandidates, ","))
	}
